...
class Call(val contact: Contact) {

    private val listeners = mutableListOf<ChatThreadListener>()

    private val _messages = mutableListOf(
        Message(1L.toString(), contact.id, "Send me a message", null, null, System.currentTimeMillis()),
        Message(2L.toString(), contact.id, "I will reply in 5 seconds", null, null, System.currentTimeMillis())
    )
